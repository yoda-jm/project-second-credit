...
- `sfx/*.wav`: synthesised from scratch by `tools/audio/rocks_sfx.py` (NumPy oscillators, noise and filters;
  no samples). Regenerate with `python3 tools/audio/rocks_sfx.py`.
- `music/cave_theme.mid`: composed as code in `tools/audio/rocks_music.py`.
- `music/cave_theme.ogg`: that MIDI rendered with FluidSynth (LGPL-2.1) and the FluidR3_GM SoundFont
  ...
  `python3 tools/audio/rocks_music.py`.
