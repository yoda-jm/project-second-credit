...
const SFX := "res://games/rocks/audio/sfx/"
...
func _ready() -> void:
	for name in ["dig", "step", "boulder_land", "boulder_roll", "gem_land", "gem_collect", "explosion", "exit_open",
			"hatch", "exit_enter", "tick", "fail", "amoeba", "magic_wall", "butterfly_burst"]:
	...
	var theme: AudioStreamOggVorbis = load("res://games/rocks/audio/music/cave_theme.ogg")
	theme.loop = true
	_music.stream = theme
