...
## ROCKS_REFERENCE=1 (tools/test.sh passes the environment through).


func test_replays_match_gdash_engine() -> void:
	if OS.get_environment("ROCKS_REFERENCE") != "1":
		print("set ROCKS_REFERENCE=1 to compare the engine with GDash on the whole corpus")
		return
