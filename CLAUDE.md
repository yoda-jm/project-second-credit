...
11. Game 8, **Iron Flags** (working title, `godot/games/flags/`): playable RTS, three campaign maps
   (`maps/campaign.flags`, generated by `tools/flags_maps.py`), territories, factories, robots, vehicles, guns, an AI
   general (`demo/flags_ai.gd`, also the demo). `--map=N`, `--zod=/path/x.map` (a Zod Engine map from the player's
   files). Open items: fog of war, APC transport, jungle and city maps, a menu to pick Zod maps.
12. Next: the shared campaign/mod system ([docs/level-packs.md](docs/level-packs.md)), the level editors, and
