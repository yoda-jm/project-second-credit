...
- `gen_elements.py` generates `godot/games/glimmerdeep/engine/cave_elements.gd` (element list, flags, BDCFF names).
- `gen_properties.py` generates `godot/games/glimmerdeep/engine/cave_properties.gd` (BDCFF cave properties and
  their defaults).

The other engine files are hand-ported and name the GDash file they come from. The MIT notice is kept in
`godot/games/glimmerdeep/engine/GDASH_LICENSE.txt`.

The reference tests (`godot/games/glimmerdeep/tests/test_*_reference.gd`) load GDash's cave files, which include
