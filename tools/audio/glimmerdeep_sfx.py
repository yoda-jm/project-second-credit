...
Usage: python3 tools/audio/glimmerdeep_sfx.py [out_dir]   (default: godot/games/glimmerdeep/audio/sfx)
...
out = sys.argv[1] if len(sys.argv) > 1 else "godot/games/glimmerdeep/audio/sfx"
...
# countdown: a clean beep for 3, 2, 1 and a bright major chord for GO
d = 0.22
save("count", np.sin(2 * np.pi * 880 * t(d)) * env(int(SR * d), 0.004, 0.12, 3) +
     0.3 * np.sin(2 * np.pi * 1760 * t(d)) * env(int(SR * d), 0.004, 0.06), 0.5)
d = 0.7
tt = t(d)
save("go", sum(np.sin(2 * np.pi * f * tt) for f in (523.3, 659.3, 784.0, 1046.5)) * env(len(tt), 0.005, 0.45, 3) +
     0.4 * bell(2093, d, decay=0.4), 0.6)
