...
def green_hell():
    """Map 3: nine jungle territories cut by two rivers, swamp everywhere, few bridges and many guns."""
    m = Map("Green Hell", "jungle", 3, 3, 20, 20, 51)
    m.rough(40, 3)
    m.outcrops(20)
    m.river_h(20, 2, [(10, 3), (48, 3)])
    m.river_h(41, 2, [(28, 3), (52, 3)])
    flags = {}
    for r in range(3):
        for c in range(3):
            flags[(c, r)] = m.flag(c, r, (c - 1) * 2, 3 if r == 0 else (-3 if r == 2 else 0))
    for r in range(3):
        m.road(flags[(0, r)], flags[(1, r)])
        m.road(flags[(1, r)], flags[(2, r)])
    m.road(flags[(0, 0)], flags[(0, 1)])
    m.road(flags[(2, 1)], flags[(2, 2)])
    m.road(flags[(1, 1)], flags[(1, 2)])
    m.road(flags[(1, 0)], flags[(1, 1)])
    m.building("fort", "blue", 0, 0, 5, 2, FORT)
    m.building("fort", "red", 2, 2, 5, 6, FORT)
    m.building("robot_factory", "neutral", 2, 0, 3, 3, RF)
    m.building("robot_factory", "neutral", 0, 2, 12, 9, RF)
    m.building("vehicle_factory", "neutral", 1, 1, 2, 2, VF)
    m.building("vehicle_factory", "neutral", 1, 0, 13, 9, VF)
    m.building("vehicle_factory", "neutral", 1, 2, 2, 2, VF)
    m.building("repair", "neutral", 0, 1, 2, 11, REPAIR)
    m.building("radar", "neutral", 2, 1, 12, 2, RADAR)
    for x, y, k in [(8, 24, "gatling"), (30, 16, "gun"), (52, 24, "gatling"), (30, 46, "gun"), (10, 46, "howitzer")]:
        m.unit("cannon", k, "neutral", x, y)
    m.unit("cannon", "gatling", "blue", 20, 6)
    m.unit("cannon", "gatling", "red", 40, 56)
    m.unit("vehicle", "jeep", "neutral", 26, 30)
    m.unit("vehicle", "tank_light", "neutral", 36, 30)
    for x, y in [(44, 52), (46, 53), (48, 52)]:
        m.unit("robot", "grunt", "red", x, y)
    for x, y in [(50, 48), (51, 49)]:
        m.unit("robot", "pyro", "red", x, y)
    for x, y in [(14, 12), (16, 13), (18, 12)]:
        m.unit("robot", "grunt", "blue", x, y)
    for x, y in [(10, 16), (11, 17)]:
        m.unit("robot", "pyro", "blue", x, y)
    m.crate("grenades", 30, 34)
    m.crate("rockets", 18, 30)
    m.props(["palm", "palm", "bush", "bush", "pine"], 140)
    return m


def rust_harbour():
    """Map 4: a ruined port city, streets on a grid, a canal with three bridges, rubble to blast through."""
    m = Map("Rust Harbour", "city", 3, 3, 21, 20, 67)
    m.rough(26, 2)
    m.outcrops(34, 1)
    m.river_h(30, 2, [(9, 4), (30, 4), (52, 4)])
    for x in range(1, m.w - 1):  # the avenues
        for y in (11, 50):
            m._road_cell(x, y)
    ...
    for r in range(3):
        for c in range(3):
            flags[(c, r)] = m.flag(c, r, 3 if c == 0 else -3, 4 if r == 0 else (-4 if r == 2 else 5))
    m.building("fort", "blue", 1, 0, 5, 1, FORT)
    m.building("fort", "red", 1, 2, 5, 6, FORT)
    m.building("robot_factory", "neutral", 0, 0, 2, 13, RF)
    m.building("robot_factory", "neutral", 2, 2, 14, 2, RF)
    m.building("vehicle_factory", "neutral", 2, 0, 14, 13, VF)
    m.building("vehicle_factory", "neutral", 0, 2, 2, 2, VF)
    m.building("repair", "neutral", 0, 1, 13, 14, REPAIR)
    m.building("radar", "neutral", 2, 1, 2, 14, RADAR)
    m.building("robot_factory", "neutral", 1, 1, 13, 2, RF)
    for x, y, k in [(10, 26, "gun"), (32, 25, "missile"), (54, 26, "gun"), (18, 36, "gatling"), (46, 36, "gatling")]:
        m.unit("cannon", k, "neutral", x, y)
    m.unit("cannon", "gatling", "blue", 36, 14)
    m.unit("cannon", "gatling", "red", 28, 48)
    for x, y, k in [(6, 20, "apc"), (56, 40, "apc"), (32, 20, "tank_medium")]:
        m.unit("vehicle", k, "neutral", x, y)
    for x, y in [(28, 56), (30, 57), (32, 56)]:
        m.unit("robot", "grunt", "red", x, y)
    for x, y in [(36, 55), (37, 56)]:
        m.unit("robot", "sniper", "red", x, y)
    m.unit("robot", "laser", "red", 26, 54)
    for x, y in [(28, 18), (30, 19), (32, 18)]:
        m.unit("robot", "grunt", "blue", x, y)
    for x, y in [(36, 17), (37, 18)]:
        m.unit("robot", "sniper", "blue", x, y)
    m.unit("robot", "laser", "blue", 26, 17)
    m.crate("rockets", 21, 40)
    m.crate("grenades", 42, 22)
    m.props(["street_lamp", "street_lamp", "ruin_pillar", "bush"], 110)
    return m


if __name__ == "__main__":
    os.makedirs(os.path.dirname(OUT), exist_ok=True)
    maps = [dust_bowl(), frost_line(), green_hell(), rust_harbour(), magma_works()]
